...
export const ProfessinalAuthLoader = async () => {
  ...
  const axiosInstance = axios.create({
    baseURL: "http://localhost:3000/api/v1/garage/professinal",
    headers: {
      ...
      if (error.response.status === 401 && !originalRequest._retry) {
        ...
        try {
          ...
          const refreshResponse = await axios.post(
            "http://localhost:3000/api/v1/garage/professinal/refresh",
          ...
          return redirect("/garage/professinal/login");
        }
      }
